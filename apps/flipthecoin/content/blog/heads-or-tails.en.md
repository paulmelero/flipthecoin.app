...
The number of heads you'd expect in _n_ tosses follows a **binomial distribution** with parameters _n_ and _p_ = 0.5. For large _n_, this approximates a normal distribution — which is why you can use coin flips to teach the Central Limit Theorem—which explains that the sum of many independent random variables follows a normal distribution (bell curve).
...
There's something philosophically interesting about the coin toss: it's accepted as fair precisely because _nobody controls it_. The coin is an arbitrator that has no stake in the outcome.
...
Ready to flip? [Try heads or tails online](https://flipthecoin.app/play) — real physics, instant result.
