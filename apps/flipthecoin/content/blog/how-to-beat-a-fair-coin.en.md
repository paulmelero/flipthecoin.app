...
Your opponent picks a pattern first; you pick second. The table gives your best choice and your chance of winning (Penney 1969; Gardner 1988):

::penney-graph
---
caption: 'Figure 1: Relationship between the eight patterns. Each arrow points from a pattern to a pattern it beats.'
---
| Opponent picks | **You pick** | Your chance of winning |
| -------------- | ------------ | ---------------------- |
| HHH            | **THH**      | 7/8                    |
| HHT            | **THH**      | 3/4                    |
| HTH            | **HHT**      | 2/3                    |
...
And there is no "best" pattern. Every pattern is beaten by another, and the relationships run in a loop: _HHH_ is crushed by _THH_; _THH_ is beaten by _TTH_; _TTH_ falls to _HTT_; _HTT_ loses to _HHT_ — which is itself beaten by _HHH_. It is **rock-paper-scissors played with coins**.

> Fairness at the level of individual tosses is no protection: predictable patterns of consecutive runs can still beat it.
...
When you pick _THH_ against your opponent's _HHH_, you are not betting on the coin. You are betting on the **transition**.
...
## Conway's leading numbers

The exact win probabilities come from a method invented by **John Horton Conway** (1937–2020), the British mathematician behind the Game of Life, surreal numbers, and _Winning Ways_. Conway's trick is to slide one pattern along the other and, at every offset where the end of one matches the beginning of the other, record a weight of a power of two. Adding up those weights gives two numbers for any pair of patterns; comparing the pattern's overlap with itself against its overlap with the opponent gives the exact odds.

We will not derive the formula here, but it is short: if the two patterns are $A$ and $B$, the odds that $A$ beats $B$ are $(BB - BA) : (AA - AB)$, where each two-letter term is a weighted overlap score. The numbers in the table above are not estimates or simulations. They are exact — a few lines of arithmetic standing in for an infinity of coin flips.

## The wait that lies

Now it gets stranger. There are several different questions you can ask about a pattern, and a fair coin answers them differently:

1. **How long does this pattern take to appear on its own?** This is the :term[expected wait]{slug='expected-wait'}, written $W$.
2. **If this pattern and another are racing, which is more likely to appear first?**

Your instinct says the pattern that arrives sooner should win the race. Often it does. But not always.

How is the expected wait computed? Each pattern carries its own self-overlap. Add up $2^r$ for every $r$ such that the last $r$ symbols of the pattern equal its first $r$ symbols:

$$W(S)=\sum_{r:\,\text{suffix}_r=\text{prefix}_r} 2^r$$

For _HHH_ the overlaps are $r=1,2,3$, so $W(HHH)=2+4+8=14$. For _THH_ only $r=3$ matches, so $W(THH)=2^3=8$. The more a pattern overlaps itself, the longer it tends to take to appear.
...
| Pattern pair     | Expected wait                | Probability the first pattern wins  |
| ---------------- | ---------------------------- | ----------------------------------- |
...
Read the second and third rows carefully. _HHH_ is the **fastest** pattern in both — expected wait 14, less than 16 — and it **loses the race** in both. Against _THHH_ it loses seven times out of eight. Against _TTHH_ it loses more than half the time.

> "Arrives sooner on average" and "wins the race" are different questions. A fair coin answers them differently.

This is the **waiting-time paradox**. It feels like a contradiction until you see why the questions separate. An expected wait averages over thousands of separate, independent runs. A race is a single shared sequence, where one pattern's arrival can block the other before it ever gets started. The average is a fact about many worlds; the race is a fact about one. It is easy to confuse them.

This paradox has a lovely formal treatment in [Frym (2023), _The Waiting-Time Paradox_](https://www.math.ucla.edu/~mason/papers/frym-WTP-published.pdf), if you want the full version.
...
- **Nontransitive dice** exist in the same way — sets of dice where each die is more likely to beat the next, around and around.
...
- **Conway's Game of Life**, the cellular automaton Conway also invented, is a cousin in spirit rather than a cycle: a handful of simple local rules, applied over and over, produce behaviour no one can predict in advance. Simple rules, endless consequences — the same lesson the coin keeps teaching.
...
Penney spent a lifetime reading sequences that someone else was trying to keep secret. It is not hard to see why a game about which pattern appears first would appeal to him: in cryptanalysis the answer never depends on the symbols alone. It depends on how one string overlaps another, where it starts, and what came just before.

What he left behind is more than a parlor trick. It is a small, perfect demonstration of a large idea: if there is no strategy that beats all the others, then "the best" is not a property a strategy has on its own. It is a property of the **relationship** between two players. Strength is relational. A pattern is strong only because of the pattern it faces; change the opponent and the same pattern becomes the loser.

This is why the waiting-time paradox and the _E. coli_ experiment are the same story. In a cyclic world, being the strongest is not a safe position — it is the position that has a predator. The deadliest strain is the one that overcommits and exposes itself. The "weakest" wins because it is nobody's target until it is too late. And the Game of Life, which Conway built from rules even simpler than a coin flip, is a reminder that complexity and surprise can come from almost nothing at all — no winner, no plan, just the next step following the last.

Nontransitivity is also the shape of most real disagreements. When A beats B, B beats C, and C beats A, no amount of careful ranking will produce a stable order — not because the players are irrational, but because the relation itself contains a cycle. Kenneth Arrow proved that any voting system with a few reasonable properties will sometimes produce exactly this loop. The failure is not in the voters. It is in the demand for a single winner.

So perhaps the oldest use of the coin — to produce one winner, cleanly, without argument — has been quietly telling us the opposite of what we assumed. A fair coin can settle a race between two things. It cannot tell you which of three things is best, because "best" may not exist. What exists is the cycle, and every position in it is someone's strength and someone else's weakness.
...
:term[Bernoulli trial]{slug='bernoulli-trial'} · :term[Binomial distribution]{slug='binomial-distribution'} · :term[Gambler's fallacy]{slug='gamblers-fallacy'} · :term[Random walk]{slug='random-walk'} · :term[Geometric distribution]{slug='geometric-distribution'}

## References

- Penney, W. "Problem 95. Penney-Ante." _Journal of Recreational Mathematics_ 2, 241, 1969. Overview: [Penney's game](https://en.wikipedia.org/wiki/Penney%27s_game).
- Honsberger, R. "Some Surprises in Probability." In _Mathematical Plums_. MAA, 1979, pp. 100–103.
- Gardner, M. "Nontransitive Paradoxes." In _Time Travel and Other Mathematical Bewilderments_. W. H. Freeman, 1988, pp. 64–66.
- Berlekamp, E. R.; Conway, J. H.; and Guy, R. K. _Winning Ways for Your Mathematical Plays, Vol. 1_, 2nd ed. A K Peters, 2001.
- Guibas, L. J. and Odlyzko, A. M. "String Overlaps, Pattern Matching, and Nontransitive Games." _Journal of Combinatorial Theory, Series A_ 30(2), 1981, pp. 183–208.
